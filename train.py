...
def train_or_eval_model(model, loss_function, kl_loss, dataloader, epoch, optimizer=None, train=False, gamma_1=1.0, gamma_2=1.0, gamma_3=1.0,int_beta=0.1):
    ...
    for data in dataloader:
        ...
        # print("umask.shape",umask.shape)#torch.Size([8, 16])
        # print("label.shape",label.shape)#torch.Size([8, 16])
        qmask = qmask.permute(1, 0, 2)
        lengths = [(umask[j] == 1).nonzero().tolist()[-1][0] + 1 for j in range(len(umask))]

        log_prob1, log_prob2, log_prob3, all_log_prob, all_prob, \
        kl_log_prob1, kl_log_prob2, kl_log_prob3, kl_all_prob,loss_int,all_final_out = model(textf, visuf, acouf, umask, qmask, lengths)
        ...
        loss = gamma_1 * loss_function(lp_all, labels_, umask) + \
            gamma_2 * (loss_function(lp_1, labels_, umask) + loss_function(lp_2, labels_, umask) + loss_function(lp_3, labels_, umask)) + \
                gamma_3 * (kl_loss(kl_lp_1, kl_p_all, umask) + kl_loss(kl_lp_2, kl_p_all, umask) + kl_loss(kl_lp_3, kl_p_all, umask))+(int_beta * loss_int)
#
    ...
    if preds!=[]:
        ...
        # print("all_final_out shape",all_final_out.shape)
    else:
    ...
    return avg_loss, avg_accuracy, labels, preds, masks, avg_fscore,all_final_out
...
if __name__ == '__main__':
    ...
    parser.add_argument('--int_gamma', type=float, default=0.00434, help="IEMOCAP=0.00434,MELD=0.056")
    parser.add_argument('--topk', type=float, default=0.7, help="IEMOCAP=0.7,MELD=0.96")
    parser.add_argument('--seed', type=int, default=1479, help="IEMOCAP=1479,MELD=2094")
    ...
    for e in range(n_epochs):
        ...
        train_loss, train_acc, _, _, _, train_fscore ,_= train_or_eval_model(model, loss_function, kl_loss, train_loader, e, optimizer, True)
        valid_loss, valid_acc, _, _, _, valid_fscore = train_or_eval_model(model, loss_function, kl_loss, valid_loader, e)
        test_loss, test_acc, test_label, test_pred, test_mask, test_fscore ,all_final_out= train_or_eval_model(model, loss_function, kl_loss, test_loader, e)
        all_fscore.append(test_fscore)

        if best_fscore == None or best_fscore < test_fscore:
            best_fscore = test_fscore
            best_label, best_pred, best_mask, best_feature = test_label, test_pred, test_mask,all_final_out
    ...
    print(classification_report(best_label, best_pred, sample_weight=best_mask,digits=4))
    print(confusion_matrix(best_label,best_pred,sample_weight=best_mask))
    #TwoD_Tsne(best_label, best_feature, best_mask, args.Dataset)
